...
module.exports.run = async (bot,message,args) => {
  ...
  if (!member) return message.channel.send('А кому выдавать?')
  message.delete();
  if (args[0]) {
    if (args[1]) {
    var num = parseInt(args[1])
    let embed = new Discord.MessageEmbed()
    .setDescription(`${message.guild.member(message.author.id)} выдал ${member} ${num} монет`)
    .setColor("#b78af5")
    bot.send(embed);
    o.coins += num
  } else {
    let embed = new Discord.MessageEmbed()
    .setDescription(`${message.guild.member(message.author.id)} Укажите кол-во монет!`)
    .setColor("#b78af5")
    bot.send(embed);
  }
}
